...
class NLNEMPloader(Dataset):
    def __init__(self, args, root_path, seq_len=1024, stride_len=1024, down_sampling_scale=1, label_type='local', flag=None, modality='Vibration', motor_id='all', normalize=True, transform=None):
        ...
        # 1. 加载数据到内存
        print(f"初始化 Dataset: {motor_id} - {self.modality} (降采样: {down_sampling_scale})")
    ...
    def __getitem__(self, idx):
        """
        返回:
        - x: Tensor, 形状 (Channels, Time_Steps) -> 适合 1D-CNN
        - y: Tensor, 标量标签 (Long)
        """
        ...
        # 转换为 Tensor
        # 原始形状: (Time, Channel) -> 目标形状: (Channel, Time)
        # PyTorch Conv1d 需要 (Batch, Channel, Length)
        x_tensor = torch.from_numpy(x_np).float().permute(1, 0)
    ...
    def _load_all_data(self, flag, cls_mode, modality, motor_id = "all", down_sampling_scale = 1, root_path = "/inspire/hdd/project/continuinglearinginlm/lijiapeng-CZXS25110021/bas_pump/data/NLN-EMP/Dataset"):
        ...
        print(f"发现 {len(experiments)} 组实验配置，开始读取...")
        ...
        print("划分训练测试集后：")
        print(f"切片前，一共有 {len(X_list)} 组数据")
    ...
    def shout(self):
        print("我是DG_data_loader_fewshot import NLNEMPloader")
