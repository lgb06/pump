...
from utils.losses import QuantizerLoss,DiscreteMaskRecLoss
...
from utils.masking import TokenMasker
import os
import time
import warnings
import numpy as np
...
class Exp_All_Task(object):
    def __init__(self, args):
        ...
        self.task_data_config = read_task_data_config(
            self.args.pretrain_data_config_path)
    ...
    def _build_model(self,ddp=False):
        ...
        self.predictor = nn.Linear(self.args.d_model,self.args.d_model).to(self.device_id)
    ...
    def _select_optimizer(self):
        ...
        # 如果存在predictor，将predictor的参数也加入到优化器中
        if hasattr(self, 'predictor'):
            predictor_params = list(self.predictor.parameters())
            all_params = model_params + predictor_params
        else:
            all_params = model_params
        model_optim = optim.Adam(all_params,
    ...
    def train(self, setting):
        ...
        pytorch_total_params = sum(p.numel() for name, p in self.model.named_parameters()
                                    if 'pretrain' not in name)
        # 改为
        self.teacher = self._build_model()# 创建一个新模型实例
        self.teacher.load_state_dict(self.model.state_dict())  # 从原始模型复制参数
        # 冻结 teacher 参数
        ...
        lr_schedule = cosine_scheduler(
            ...
            self.args.min_lr,
            self.args.pretrain_epochs, train_steps,
        ...
        # else:
        # criterion = DistillLoss(d_model=self.args.d_model,
        #                         student_temp=self.args.student_temp,
        #                         teacher_temp=self.args.teacher_temp,
        #                         center_momentum=self.args.center_momentum).to(self.device_id)

        criterion = SimDINOLoss(d_model=self.args.d_model,alpha=self.args.cls_weight,
                                student_temp=self.args.student_temp,
                                ).to(self.device_id)
        scaler = NativeScaler()

        for epoch in tqdm(range(self.args.pretrain_epochs), desc="Training Out Epochs"):
    ...
    def train_one_epoch(self, model_optim, data_loader_cycle, criterion, epoch, train_steps, scaler, lr_schedule):
        ...
        for i, (sample_init, task_id) in enumerate(tqdm(data_loader_cycle, desc="Training Inner Epoch {}".format(epoch + 1))):
            ...
            for sample_idx in range(len_sample_list):
                ...
                x_enc_mask,mask_id =self.mask(x_enc)
                with torch.cuda.amp.autocast():
                    st_cls,st_seq = self.model(
                        x_enc=x_enc_mask, x_mark_enc=x_mark_enc, task_id=task_id, task_name=task_name, enable_mask=True)
                    st_cls = self.predictor(st_cls)
                    st_seq = self.predictor(st_seq)
                    te_cls,te_seq = self.teacher(
                        x_enc=x_enc, x_mark_enc=x_mark_enc, task_id=task_id, task_name=task_name, enable_mask=True)
                    # Match teacher's precision to student's
                    te_cls = te_cls.to(st_cls.dtype)
                    te_seq = te_seq.to(st_seq.dtype)
                    
                    out = {'st_seq': st_seq, 'st_cls': st_cls,
                           'te_seq': te_seq, 'te_cls': te_cls,
                           'mask_id': mask_id}
            ...
            #         'train_seq_loss_'+self.task_data_config_list[task_id][0]: loss_dict['seq_loss'].item(),
            #         'train_tcr_loss_'+self.task_data_config_list[task_id][0]: loss_dict['tcr_loss'].item(),
            #         'train_ft_loss_'+self.task_data_config_list[task_id][0]: loss_dict['cls_ft'].item(),
            #         'train_ff_loss_'+self.task_data_config_list[task_id][0]: loss_dict['cls_ff'].item(),
            #         'train_sum_loss_'+self.task_data_config_list[task_id][0]: loss_dict['total_loss'].item(),
    ...
    def get_multi_source_data(self, this_batch, task_name, small_batch_size, min_keep_ratio=None):
        ...
        elif 'PHM' in task_name:
            batch_x, batch_x_mark,padding_mask = this_batch
            batch_x = batch_x.float().to(self.device_id)
            batch_x_mark = batch_x_mark.float().to(self.device_id)
