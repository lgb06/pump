...
class Model(nn.Module):
    def __init__(self, args, configs_list, pretrain=False):
        ...
        # Initial layers
        self.conv1 = nn.Conv1d(self.channel_num, 16, kernel_size=17, stride=1, padding=8)
        ...
        self.conv4 = nn.Conv1d(64, 128, kernel_size=2, stride=1, padding=1)
        ...
        self.rul_predictor = nn.Linear(128, 1)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec=None, x_mark_dec=None, mask=None, task_id=None, task_name=None, enable_mask=None):
        ...
        if 'classification' in task_name:
            dec_out = self.classifier(x4)
            return dec_out
        if 'RUL' in task_name:
            dec_out = self.rul_predictor(x4)
            return dec_out
        return None
