...
class Model(nn.Module):
    def __init__(self, args,configs_list,pretrain=True):
        ...
        self.conv1 = nn.Conv1d(self.channel_num, 32, kernel_size=4, stride=1)
        self.conv2 = nn.Conv1d(32, 64, kernel_size=4, stride=1)
        self.conv3 = nn.Conv1d(64, 128, kernel_size=4, stride=1)
        self.conv4 = nn.Conv1d(128, 256, kernel_size=4, stride=1)
        self.conv5 = nn.Conv1d(256, 512, kernel_size=4, stride=1)

        self.classifier = nn.Linear(1040896, self.num_class)
        self.rul_predictor = nn.Linear(1040896, 1)

    def forward(self, x_enc, x_mark_enc, x_dec=None, x_mark_dec=None, mask=None, task_id=None, task_name=None, enable_mask=None):
        x_enc = x_enc.permute(0, 2, 1)  # Adjusting to match input size [B, D, L]
        x = torch.relu(self.conv1(x_enc))
        x = torch.relu(self.conv2(x))
        x = torch.relu(self.conv3(x))
        x = torch.relu(self.conv4(x))
        x = torch.relu(self.conv5(x))
        out = x.view(x.size(0), -1)  # Flatten the output tensor
